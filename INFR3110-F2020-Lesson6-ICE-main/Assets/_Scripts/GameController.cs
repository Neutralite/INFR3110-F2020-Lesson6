...
public class GameController : MonoBehaviour
{
    ...
    public bool AddTile(Vector3 new_location)
    {
        ...
        if (!m_tilePosition.ContainsKey(key))
        {
            ...
            {
                // replaces var newTile = TileFactory.Instance().CreateTile(new_location);
    ...
    private void _Reset()
    {
        if (Tiles.Count > 0)
        {
            ...
            {
                tileManager.ReturnTile(child.gameObject); // replaces Destroy(child.gameObject);
            }
        }
